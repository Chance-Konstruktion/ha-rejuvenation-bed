...
class BiorhythmusCurve:
    ...
    PHASE_LANDING = 0.15      # 0.00 - 0.15: Einschlafphase
    PHASE_DEEP_SLEEP = 0.50   # 0.15 - 0.65: Tiefschlaf
    PHASE_TRANSITION = 0.20   # 0.65 - 0.85: Übergangsphase
    PHASE_WAKE = 0.15         # 0.85 - 1.00: Aufwachphase
    ...
    def _calculate_temperature(self, t: float) -> float:
        ...
        if t < self.PHASE_LANDING:
            ...
            return self._lerp(
                self.sleep_temp,
                self.deep_sleep_temp,
                self._smooth_cos(phase_t)
            )
