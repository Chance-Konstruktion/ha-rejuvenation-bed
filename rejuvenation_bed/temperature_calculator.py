...
class TemperatureCalculator:
    ...
    async def async_calculate_target(
        ...
        is_present: bool = False,  # NEU: Präsenz-Status
        has_presence_sensor: bool = False,  # NEU: Hat Zone Präsenz-Sensor?
    ) -> float:
        """
        Zentrale Funktion: Berechnet die Zieltemperatur für eine Zone.
        
        NEU: Präsenz-basierte Kurven-Aktivierung!
        - Mit Präsenz-Sensor: Kurve startet wenn Person im Bett
        - Ohne Präsenz-Sensor: Kurve orientiert sich an geplanter Schlafzeit
        
        Args:
            zone_index: Index der Zone (0 oder 1)
            energy_state: Dict vom EnergyCalculator
            veto_state: Dict mit Veto-Checks (z.B. Sommer-Modus)
            coordinator: Optional - der Coordinator (für manuelle Overrides)
        ...
        # NEU: Warmhalte-Zeitfenster statt nur bedtime
        # Options überschreiben Config (Options = Einstellungen in der UI geändert)
        ...
        # Prüfe ob wir im Warmhalte-Zeitfenster sind
        in_warm_window = self._is_in_time_range(current_time, warm_from, warm_until)
        
        if has_presence_sensor:
            # MIT Präsenz-Sensor: Kurve startet bei Präsenz innerhalb des Zeitfensters
            if is_present:
                use_biorhythmus = True
                _LOGGER.debug(f"Zone {zone_index}: Präsenz erkannt → Biorhythmus aktiv")
            elif in_warm_window:
                # Im Zeitfenster aber niemand da: Vorheizen/Warmhalten
                preheat_temp = curve.sleep_temp
                _LOGGER.debug(
                    f"Zone {zone_index}: Im Warmhalte-Fenster (wartet auf Präsenz) → {preheat_temp}°C"
                )
                return preheat_temp
            else:
                # Außerhalb Zeitfenster und leer: Standby
                _LOGGER.debug(f"Zone {zone_index}: Außerhalb Warmhalte-Fenster → Standby {standby_temp}°C")
                return max(standby_temp, self.min_temp)
        else:
            # OHNE Präsenz-Sensor: Zeitbasiert
            if in_warm_window:
                # Im Warmhalte-Zeitfenster = Biorhythmus aktiv
                use_biorhythmus = True
                _LOGGER.debug(f"Zone {zone_index}: Im Warmhalte-Fenster → Biorhythmus aktiv")
            else:
                # Außerhalb des Fensters: Standby-Temperatur
                # Aber: Vorheizen starten wenn es bald losgeht
                minutes_to_warm_start = self._minutes_until_time(current_time, warm_from)
                
                # Vorheizzeit basierend auf Bett-Typ
    ...
    async def _ensure_resolvers_initialized(self, zone_index: int):
        ...
        options = self.config_entry.options
        zone_options = options.get(f"zone_{zone_index}", {})
        ...
        # Wake-Time: Zuerst aus Zone-Optionen, dann Zone-Config, dann global warm_until
        wake_time_str = zone_options.get("wake_time") or zone_conf.get("wake_time") or warm_until_str
        wake_time_fixed = self._parse_time(wake_time_str)
        
        # Wake-Mode und Alarm-Entity
