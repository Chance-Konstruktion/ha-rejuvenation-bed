...
class TemperatureCalculator:
    ...
    async def async_calculate_target(
        ...
        warm_from_str = (
            options.get(f"{prefix}warm_from") or options.get("warm_from") or global_conf.get("warm_from", "22:00")
        )
        warm_until_str = (
            options.get(f"{prefix}warm_until") or options.get("warm_until") or global_conf.get("warm_until", "07:00")
        )
        ...
        if has_presence_sensor:
            ...
            if post_alarm and is_present:
                ...
                _LOGGER.debug(f"Zone {zone_index}: Post-Alarm Warmhalten → {wake_temp}°C")
                return wake_temp
            ...
            if actual_bedtime is not None:
                ...
                _LOGGER.debug(
                    f"Zone {zone_index}: Biorhythmus aktiv " f"(Einschlafzeit: {actual_bedtime.strftime('%H:%M')})"
                )

            elif in_warm_window:
                # Im Zeitfenster aber noch nicht im Bett: Vorheizen
                preheat_temp = curve.sleep_temp if curve else 28.0
                _LOGGER.debug(f"Zone {zone_index}: Warmhalte-Fenster aktiv, wartet auf Präsenz → {preheat_temp}°C")
                return preheat_temp
                ...
                if 0 < minutes_to_warm_start <= preheat_minutes:
                    ...
                    return self._charge_standby_temp(standby_temp, energy_state, zone_index)
            ...
            if in_warm_window:
                ...
                if 0 < minutes_to_warm_start <= preheat_minutes:
                    preheat_temp = curve.sleep_temp if curve else 28.0
                    _LOGGER.debug(f"Zone {zone_index}: Vorheizen ({minutes_to_warm_start} Min) → {preheat_temp}°C")
                    return preheat_temp
                else:
        ...
        if sleep_stage_resolver and sleep_stage_resolver.should_override_curve():
            ...
            _LOGGER.debug(f"Zone {zone_index}: Wearable-Override aktiv " f"(Offset: {sleep_stage_offset:+.1f}°C)")
    ...
    def _charge_standby_temp(self, base_temp: float, energy_state: dict, zone_index: int) -> float:
        """
    ...
    async def _ensure_resolvers_initialized(self, zone_index: int):
        ...
        prefix = f"zone_{zone_index}_"
        zone_options = {k[len(prefix) :]: v for k, v in options.items() if k.startswith(prefix)}

        # Warmhalte-Zeitfenster: Zone → Global Options → Global Config → Default
        warm_from_str = (
            zone_options.get("warm_from") or options.get("warm_from") or global_conf.get("warm_from", "22:00")
        )
        warm_until_str = (
            zone_options.get("warm_until") or options.get("warm_until") or global_conf.get("warm_until", "07:00")
        )
        ...
        alarm_entity = (
            zone_options.get("alarm_entity") or zone_conf.get("alarm_entity") or global_conf.get("alarm_entity")
        )
        ...
        _LOGGER.info(f"Zone {zone_index}: Aufwachzeit = {wake_time.strftime('%H:%M')} " f"(Quelle: {wake_source})")
        ...
        sleep_stage_resolver = SleepStageResolver(
            self.hass, sleep_stage_entity=sleep_stage_entity, enabled=sleep_stage_entity is not None
        )
    ...
    def update_trend_data(self, zone_index: int, current_temp: float):
        ...
        if zone_index not in self._temp_history:
            self._temp_history[zone_index] = {"last_val": current_temp, "trend": 0.0}
            return
        ...
        self._temp_history[zone_index]["trend"] = self._temp_history[zone_index]["trend"] * 0.7 + diff * 0.3
