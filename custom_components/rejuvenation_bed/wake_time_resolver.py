...
class WakeTimeResolver:
    ...
    def __init__(
        ...
        if mode == "alarm_entity" and not alarm_entity:
            _LOGGER.warning("Modus 'alarm_entity' gewählt, aber keine Entity angegeben. " "Fallback auf 'fixed_time'.")
    ...
    def _resolve_alarm(self) -> Tuple[time, str]:
        ...
        # Fallback
        _LOGGER.warning(
            f"Konnte Wecker-Zeit von '{self.alarm_entity}' nicht lesen. " "Verwende feste Zeit als Fallback."
        )
    ...
    def _read_alarm_entity(self) -> Optional[time]:
        ...
        if domain == "sensor":
            ...
            _LOGGER.warning(
                f"Alarm-Entity-Typ '{domain}' wird noch nicht unterstützt. " "Bitte melde dies als Feature-Request!"
            )
            return None
    ...
    def _parse_sensor_alarm(self, state: State) -> Optional[time]:
        ...
        try:
            ...
            _LOGGER.warning(
                f"Konnte Zeitformat von '{state.state}' nicht parsen. " "Erwartet: ISO Timestamp oder HH:MM."
            )
            return None
    ...
    def _parse_input_datetime(self, state: State) -> Optional[time]:
        ...
        try:
            ...
            _LOGGER.warning(f"Konnte input_datetime '{state.state}' nicht parsen.")
            return None
