...
class SleepStageResolver:
    ...
    def __init__(self, hass: HomeAssistant, sleep_stage_entity: Optional[str] = None, enabled: bool = True):
        """
        ...
        if enabled and not sleep_stage_entity:
            _LOGGER.info(
                "Sleep-Stage-Resolver aktiviert, aber keine Entity konfiguriert. " "Fallback auf zeitbasierte Kurve."
            )
            self.enabled = False
    ...
    def get_current_stage(self) -> Tuple[SleepStage, str]:
        ...
        if not state:
            _LOGGER.debug(f"Schlafphasen-Sensor '{self.sleep_stage_entity}' nicht gefunden.")
