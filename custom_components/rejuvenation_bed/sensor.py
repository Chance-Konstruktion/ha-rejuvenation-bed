...
class BedLegacyComparisonSensor(RejuvenationBedSensorBase):
    ...
    Traditionell (80er Jahre Technik):
    - Heizung läuft 24/7 mit Thermostat
    - Typisch: 60-80% Einschaltdauer
    - Keine Nachtabsenkung, kein Solar-Nutzen
    
    Smart (Rejuvenation Bed):
    - Biorhythmus-Kurve (weniger heizen in der Nacht)
    - Solar-Boost (kostenloser Strom)
    - Präsenz-basiert (nicht heizen wenn niemand da)
    """
    ...
    def extra_state_attributes(self):
        ...
        # Kosten (geschätzt 0.30€/kWh)
        price_per_kwh = 0.30
        yearly_savings_eur = (legacy_yearly - smart_yearly) * price_per_kwh
        ...
        return {
            ...
            "tracking_tage": days,
            "berechnung": (
                "Legacy = 70% Einschaltdauer × 24h × Leistung. "
                "Smart = Tatsächliche Heizzeit × Leistung."
            ),
        }
