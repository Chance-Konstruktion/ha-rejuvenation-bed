...
class SleepScoreCalculator:
    ...
    def _calc_temperature_stability(self, night: NightData) -> int:
        ...
        if std_dev <= self.TEMP_TOLERANCE_EXCELLENT:
            ...
            score = 100 - (std_dev - self.TEMP_TOLERANCE_EXCELLENT) / (self.TEMP_TOLERANCE_GOOD - self.TEMP_TOLERANCE_EXCELLENT) * 20
        elif std_dev <= self.TEMP_TOLERANCE_ACCEPTABLE:
            # Linear interpolieren 80 → 50
            score = 80 - (std_dev - self.TEMP_TOLERANCE_GOOD) / (self.TEMP_TOLERANCE_ACCEPTABLE - self.TEMP_TOLERANCE_GOOD) * 30
        else:
    ...
    def _calc_curve_adherence(self, night: NightData) -> int:
        ...
        deviations = [
            abs(actual - target) 
            for actual, target in zip(night.temp_readings, night.target_temps)
        ]
        ...
        if night.nadir_time_actual and night.nadir_time_expected:
            nadir_diff_minutes = abs(
                (night.nadir_time_actual - night.nadir_time_expected).total_seconds() / 60
            )
