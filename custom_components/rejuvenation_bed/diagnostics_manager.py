...
class DiagnosticsManager:
    ...
    def get_thermal_summary(self, zone_index: int, coordinator) -> dict:
        """
    ...
    def get_energy_budget(self) -> dict:
        ...
        return {
            ...
            "total_kwh": round(budget["solar_kwh_used"] + budget["grid_kwh_used"], 2),
            "total_runtime_hours": round(budget.get("total_runtime_hours", 0.0), 1),
            "solar_percentage": round(
                (budget["solar_kwh_used"] / max(budget["solar_kwh_used"] + budget["grid_kwh_used"], 0.001)) * 100, 1
            ),
            ...
            "avg_kwh_per_day": round(
                (budget["solar_kwh_used"] + budget["grid_kwh_used"]) / max(days_since_reset, 0.04), 2
            ),
    ...
    def update_energy_usage(
        ...
        if corrected_watts > 5:
            ...
            self._energy_budget["total_runtime_hours"] = (
                self._energy_budget.get("total_runtime_hours", 0) + runtime_hours
            )
    ...
    def log_health_event(self, severity: str, category: str, message: str, details: Optional[dict] = None):
        """
    ...
    def get_complete_diagnostics(self, coordinator) -> dict:
        ...
        for zone_idx in range(zones_count):
            thermal_summaries[f"zone_{zone_idx}"] = self.get_thermal_summary(zone_idx, coordinator)
        ...
        return {
            ...
            "anti_short_cycle": (
                coordinator.anti_short_cycle_manager.get_diagnostics()
                if hasattr(coordinator, "anti_short_cycle_manager")
                else {}
            ),
            "timestamp": local_now().isoformat(),
        }
