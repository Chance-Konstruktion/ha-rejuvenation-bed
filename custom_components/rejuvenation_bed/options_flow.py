...
class RejuvenationBedOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_global_settings(self, user_input=None):
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                ...
                vol.Optional(
                    "co2_sensor",
                    description={"suggested_value": current.get("co2_sensor", global_data.get("co2_sensor"))}
    ...
    async def async_step_edit_zone_settings(self, user_input=None):
        ...
        schema_dict[vol.Required(
            "wake_mode",
            default=_get("wake_mode", "fixed")
        )] = selector.SelectSelector(
            selector.SelectSelectorConfig(
                options=[
                    {"value": "fixed", "label": "⏰ Feste Aufwachzeit"},
                    {"value": "alarm", "label": "📱 Wecker-Entity"},
                    {"value": "hybrid", "label": "🔀 Hybrid (Wecker wenn gesetzt, sonst fest)"},
                ],
                mode=selector.SelectSelectorMode.LIST
            )
        )
        ...
        schema_dict[vol.Optional(
            "wake_time",
            default=_get("wake_time", "07:00")
        )] = selector.TimeSelector()

        # Wecker-Entity (optional)
