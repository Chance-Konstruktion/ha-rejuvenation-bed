...
class RampController:
    ...
    def should_start_preheat(
        ...
        preheat_time = self.calculate_preheat_time(current_temp, target_temp, power_watts, water_liters)
        ...
        if now >= start_time:
            ...
            return (
                True,
                f"Vorheizen starten! {preheat_time.total_seconds()/60:.0f} Min nötig, noch {minutes_until_bed:.0f} Min bis Schlafzeit",
            )
        else:
    ...
    def force_setpoint(self, zone_index: int, temp: float):
        ...
        """
        _LOGGER.warning(f"Zone {zone_index}: Setpoint erzwungen auf {temp}°C (Rampe übersprungen!)")
    ...
    def get_state_for_storage(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_update": {str(k): v.isoformat() for k, v in self._last_update.items()},
            "ramp_start": {str(k): v.isoformat() for k, v in self._ramp_start.items()},
    ...
    def restore_state_from_storage(self, data: Dict[str, Any]):
        ...
        try:
            ...
            _LOGGER.info(f"RampController: State wiederhergestellt " f"({len(self._last_setpoint)} Zonen)")
