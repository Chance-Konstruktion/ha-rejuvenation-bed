...
from .const import DOMAIN, DEFAULT_POWER, DEFAULT_SUMMER_TEMP
...
class RejuvenationBedConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_zone_config(self, user_input=None):
        ...
        if user_input is not None:
            ...
            _LOGGER.info(f"Zone {current_zone_index}: Hardware-Level '{hardware_level}' erkannt")
        ...
        # Zonen-Label für UI
        zone_labels = {"main_zone": "Hauptmatratze", "left_side": "Linke Seite", "right_side": "Rechte Seite"}
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Required("hardware_max", default=36): vol.All(vol.Coerce(int), vol.Range(min=25, max=38)),
                    vol.Required("boost_target_temp", default=34): vol.All(vol.Coerce(int), vol.Range(min=25, max=38)),
    ...
    async def async_step_hardware_summary(self, user_input=None):
        ...
        for idx, zone in enumerate(self._data["zones"]):
            ...
            if level == "E":
                ...
                features_available.append(f"✅ {zone_name}: TEMPERATUR")
                features_available.append("   • Biorhythmus-Kurve mit Schlafphasen")
                features_available.append("   • Präsenz-Erkennung (Temperatur-Varianz)")
                features_available.append("   • Manueller Boost")
                features_unavailable.append(f"⚠️ {zone_name}: Kein Power-Sensor")
                features_unavailable.append("   → Energie-Statistiken nicht verfügbar")
                features_unavailable.append("   → Solar-Batterie eingeschränkt")
                features_unavailable.append("   → Tipp: Power-Sensor nachrüsten für vollen Umfang!")
            elif level == "B":
        ...
        return self.async_show_form(step_id="hardware_summary", description_placeholders={"features": summary_text})
    ...
    async def async_step_energy_config(self, user_input=None):
        ...
        has_power_sensor = any(zone.get("power_sensor") for zone in self._data["zones"])

        # Berechne Gesamt-Nennleistung aus Zonen
        total_power_rating = sum(zone.get("power_rating", DEFAULT_POWER) for zone in self._data["zones"])
    ...
    async def async_step_global_config(self, user_input=None):
        ...
        if user_input is not None:
            ...
            if "energy" in self._data and "electricity_price" in self._data["energy"]:
                self._data["global"].setdefault("electricity_price", self._data["energy"]["electricity_price"])
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Optional("alarm_entity"): selector.EntitySelector(
                        selector.EntitySelectorConfig(domain=["sensor", "input_datetime"], multiple=False)
                    ),
